function DotMatrixChart(dataset,options){
	...
	var numberOfColumns = noOfGroupsInARow*noOfCirclesInARow + noOfGroupsInARow*3;
    var numberOfLines = maxNoOfLinesInGroup * numberOfGroupInColumn + numberOfGroupInColumn*4;
    ...
    var	margin = {top: dotRadius*10, right: dotRadius*1, bottom: dotRadius*10, left: dotRadius*1};
    ...
    function generate_array(d){
		...
		var delim_y = Math.floor(groupScale(d.group)/noOfGroupsInARow) + 4;
	...
	labels.append('text')
		...
		.attr("x", function(d,i){return xScale(i%noOfGroupsInARow * (noOfCirclesInARow+2));})
		.attr("y", function(d,i){return yScale((Math.floor(i/noOfGroupsInARow + 1)) * (maxNoOfLinesInGroup + 2));});
